...
class FoodDetector:
    ...
    def ask_ai(self, food_name):
        ...
        try:
            ...
            6. "water_usage": The amount of water used to produce the food (e.g., "10L/kg")
            Example format:
...
def detect_food_base64():
    ...
    try:
        print("🍎 Received request to /detect-base64")
        ...
        print(f"🖼️ Decoded image size: {len(image_bytes)} bytes")

        # Detect food
        result = detector.detect_food(image_bytes)
        print(f"✅ Detection result: {result.get('message', 'No message')}")
